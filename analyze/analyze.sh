...
# 检查是否提供了参数
if [ $# -eq 0 ]; then
    echo -e "${ERROR} Missing parameter for analyze command.${NC}" | tee ./result/analyze.log >&2
...
case "$command" in
    eleQ)
        echo "Calculating CPM electrode charge..."  | tee ./result/analyze.log >&2
        bash ${bash_dir}/source/get_eleQ.sh  | tee -a ./result/analyze.log >&2
        ;;
    onfly)
        echo "calculate onfly data..."  | tee ./result/analyze.log >&2
        bash ${bash_dir}/source/get_onfly.sh  | tee -a ./result/analyze.log >&2
        ;;
    mdheat)
        echo "calculate mdheat data..."  | tee ./result/analyze.log >&2
        bash ${bash_dir}/source/get_mdHeat.sh  | tee -a ./result/analyze.log >&2
        ;;
    MD_PP)
        echo "Run the MD post-processing module..." | tee ./result/analyze.log >&2
        bash ${bash_dir}/source/MD_PP.sh | tee -a ./result/analyze.log >&2
        ;;
    *)
        echo -e "${ERROR}not found command: $command" | tee ./result/analyze.log >&2
